...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = (chars.sortWith(_<_)).group().map(xs => (xs.head, xs.length))
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((_,_) match {case ((c, fc), (d, fd)) => fc < fd}).map(_ match {case (c,w) => new Leaf(c,w)})
