...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toList.groupBy(_.toLower).map(_ match {case (a,b) => (a, b.length)}).toList.sortWith(_._1 < _._1)
   ...
   */
  def subtract(occurrencesx: Occurrences, occurrencesy: Occurrences): Occurrences = occurrencesx.map(x => occurrencesy.find(_._1 == x._1) match {
