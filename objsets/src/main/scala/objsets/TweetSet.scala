...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = if (!that.isEmpty) incl(that.head).union(that.tail) else this

  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = if (isEmpty) new EmptyTrending() else new NonEmptyTrending(findMin, remove(findMin).ascendingByRetweet)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = tail.filter0(p, if (p(head)) accu.incl(head) else accu)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(t.text.contains(_)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(t.text.contains(_)))
